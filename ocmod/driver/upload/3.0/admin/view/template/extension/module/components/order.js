...
export default defineComponent({
  ...
  template: `
    <div class="w-full rounded-md text-black bg-white" :class="order.revert_order && !disableRo && 'border-4 border-red-500'">
      <!-- Header -->
      <div 
      ...
      >
        ...
        <div class="flex justify-between items-center">
          <div class="flex items-center w-8/12 gap-2">
            ...
            <div>
              <!-- Order id -->
              <label @click.stop="" :for="'order-'+order.order_id" class="text-base text-primary font-semibold break-words">Order #{{ order.order_id }} <span v-if="addedBy">- {{ addedBy }}</span></label>
              <!-- DO -->
            ...
            <div class="w-full flex gap-2">
              ...
              <div class="w-10/12 text-sm">
                ...
                <div class="text-sm font-medium text-primary capitalize">
                  <!-- Weight -->
                  <p v-if="product.show_weight">{{ product.order_weight }} kg</p>
                  <!-- Option 1-->
            ...
            <div v-if="showWarehouseQty || showFinalQty" class="mt-4 flex flex-col gap-4 text-sm font-medium text-gray-700">
              ...
              <div v-if="showWarehouseQty" class="flex items-center justify-between">
                ...
                <div class="w-7/12 flex justify-between items-center border-2 text-gray-700 border-gray-400 rounded-md">
                  <input 
                    type="number" 
                    v-if="(showWarehouseQty && !product.out_of_stock) || showFinalQty"
                  ...
                  <!-- OOS -->
                  <input 
                    v-if="!showFinalQty && product.out_of_stock"
                    type="text" 
              ...
              <div v-if="product.show_weight" class="flex items-center justify-between">
                <p>Weight (kg) :</p>
                <!-- Add Weight -->
                <div v-if="!disableUpdate" class="w-fit ml-auto mr-2">
                ...
                <div class="w-7/12 flex justify-between items-center border-2 border-gray-400 text-gray-700  rounded-md bg-gray-300">
                  <div v-if="!product.out_of_stock" class="w-7/12 flex flex-col border-r-2 border-black/60">
                    <div class="relative" v-for="(pw, i) in product.partial_weights" >
                      <input
                        ...
                        :disabled="disableUpdate || i != product.partial_weights.length - 1"
                        @focus="$event.target.select()"
                      ></input>
                      <!-- Remove Weight -->
                      <div v-if="product.partial_weights.length > 1 && !disableUpdate" @click="removeWeight(i, product)" class="absolute right-1 top-1/2 -translate-y-1/2">
              ...
              <div class="flex justify-end gap-2 text-black/80" :class="(disableUpdate && product.out_of_stock ? 'hidden' : '')">
                <input 
                  type="checkbox" 
                  @change="handleOutOfStock($event.target.checked, product)" 
  ...
  setup(props, { slots }) {
    ...
    watch(
      () => props.products.map(product => product.partial_weights),
      (newWeights, oldWeights) => {
        newWeights.forEach((weights, index) => {
          props.products[index].weight = weights.reduce((sum, weight) => sum + weight, 0);
        });
      },
      { deep: true }
    );

    const handleOutOfStock = (checked, product) => {
      if (checked) {
        if (!props.disableWarehouseQty) {
        ...
        if (!props.disableWarehouseQty) {
          product.warehouse_qty = product.quantity;
        }
        product.dc_qty = product.quantity;
        product.weight = product.order_weight;
      }
    ...
    const isPrimary = computed(() => {
      const [day, month, year] = props.order.delivery_date.split('/').map(Number);
